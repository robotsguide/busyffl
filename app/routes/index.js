  ...
  getTradeData(trades, data) {
    trades.forEach(item => {
      ...
      fromPlayer.forEach(id => {
        const roster = data.teamRosters.findBy('id', id);
        if(!Ember.isNone(roster)) {
          roster.set('player', data.players.findBy('id', roster.get('playerId')));
          fromPlayerArray.pushObject(roster);
        }
      });
      ...
      toPlayer.forEach(id => {
        ...
        if(!Ember.isNone(roster)) {
          roster.set('player', data.players.findBy('id', roster.get('playerId')));
          toPlayerArray.pushObject(roster);
        }
      });
